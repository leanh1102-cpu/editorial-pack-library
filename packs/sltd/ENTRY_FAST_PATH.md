...
rules/sltd_living_world_community_motion_gate.md if social/community/animal/work habit changes are in scope
rules/sltd_narrative_beat_escalation_aftershock_gate.md if residue lacks cost, result, aftershock, or return path
rules/sltd_prose_craft_style_material_gate.md if residue becomes label, mood, or exposition
...
If Đại Phong's private question, inner reasoning, curiosity pressure, small test, kept detail, clue chain, small secret, supernatural pressure, or inquiry route is in scope, run `protagonist_inquiry_check.md`.

If Đại Phong's development path, protagonist gain, growth route, gain type, cost, knowledge limit, forbidden gain, world-rule sensitivity, or benchmark-level protagonist appeal is in scope, run `protagonist_advancement_check.md`.
